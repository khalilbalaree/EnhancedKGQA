...
class EntityPredictor(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        all_permute = True
        possible_relations = torch.where(self.sigmoid(cls_output)>0.5,1.0,0.0)# print(questions, relation_indices)
        pred = complex.score(self.head_embedding_list, rel_embedding, self.candidate_embedding)

        #r_score = torch.tensor(kg.relation_matching(possible_relations, self.head_words, self.sigmoid(pred))).to(dev)

        batch_onehot = []
        group_onehot = []
        count = 0

        relation_label = torch.tensor(kg.path_relations_batch(self.head_words, ans_label, this_hop_type)).to(dev)
        for i, r in enumerate(possible_relations):
            #print("###############################")
            #print("correct relation pos, (not ordered): ",torch.where(relation_label[count] > 0))
            potiential_relations = []
            this_candidates = []
            all_poss = []
            r = torch.nonzero(r).squeeze(1).tolist()
            #print("relation positions are: ", r)
            if all_permute:
                for this_k in (1,self.hop):
                    all_poss += [p for p in itertools.product(r, repeat=this_k)]
            else:
                all_poss = [p for p in itertools.product(r, repeat=self.hop)]

            group_onehot_batch = torch.zeros(43234).unsqueeze(0).to(dev)
            for each_permute in all_poss:
                last_this_candiates = kg.candidates2onehot(set(this_candidates.copy()))
                kg.find_tails_from_head(self.head_words[i], each_permute, this_candidates)
                current_this_canadiates = kg.candidates2onehot(set(this_candidates))

                update_this_canadiates = current_this_canadiates - last_this_candiates
                if (update_this_canadiates).sum() > 0:
                    effective_rel_onehot = torch.tensor(update_this_canadiates, dtype=torch.float).to('cuda')
                    group_onehot_batch = torch.cat((group_onehot_batch, effective_rel_onehot.unsqueeze(0)), dim=0)
                    #ans = torch.dot(ans_label[count],effective_rel_onehot)
                    #print("each permute info: ",ans,torch.sum(effective_rel_onehot),each_permute)
                    #rel_embedding_permuted = torch.zeros(400).unsqueeze(0).to(dev)
                    #for r in each_permute:
                        #a_rel_emb = self.relation_embedding[r] # 400 dim
                        #rel_embedding_permuted = torch.cat((rel_embedding_permuted,a_rel_emb.unsqueeze(0)),dim= 0)
                    #rel_embedding_permuted = rel_embedding_permuted[1:]

                    #if ans > 0:
                        #potiential_relations.append(each_permute)
            #print("potiential relation pos that contains ans: ",potiential_relations)

            this_onehot = kg.candidates2onehot(set(this_candidates))
            #print(this_onehot.sum())
            batch_onehot.append(this_onehot)
            group_onehot_batch = group_onehot_batch[1:]
            group_onehot.append(group_onehot_batch)
            count +=1

        onehot = torch.tensor(np.stack(batch_onehot)).to(dev)
        #torch.set_printoptions(sci_mode=False)
        #print("#################")
        #print(torch.sum(onehot,dim=1))
        ...
        sigmoid_pred = (self.sigmoid(pred)) * onehot
        topk = 3
        local_count = 0
        acc = 0
        acc1 = 0
        ...
        acc = 0
        acc1 = 0
        for each_group in group_onehot:
            for each_one_hot_seg in each_group:
                find = False
                #r_score = torch.tensor(
                    #kg.relation_matching(possible_relations, self.head_words, self.sigmoid(pred)*each_one_hot_seg)).to(dev)

                sigmoid_pred = self.sigmoid(pred[local_count]) * each_one_hot_seg #+ r_score[local_count]) * each_one_hot_seg#
                values,ind = torch.topk(sigmoid_pred, 3)
                index =0
                for i in ind:
                    if ans_label[local_count][i] == 1:
                        acc1 += 1
