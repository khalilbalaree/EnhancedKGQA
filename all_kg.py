...
class KG():
    ...
    def path_relations(self, start, goal, k):
        ...
        try:
            sps = self.path_relation_cache['->'.join([start, goal])]
        except:
            sps = self.get_path_fixed_length(start, goal, k)
            self.path_relation_cache['->'.join([start, goal])] = sps
        ...
        try:
            ...
            for ea in pathGraph.edges():
                r = self.graph1.edges[ea[0], ea[1], 0]['weight']  # assuming kg is sparse
    ...
    def relation_matching(self, starts, k):
        # return score
        rel_scores = []
        for start in starts:
            try:
                rel_score = self.path_relation_all_cache[start]
            except:
                rel_score = []
                for e in self.entity_dict.keys():
                    path_one_hot = self.path_relations(start, e, k)
                    rel_score.append(path_one_hot)
                self.path_relation_all_cache[start] = rel_score
...
# kg = KG()
# a = list()
# b = kg.find_tails_from_head('James Glickenhaus',(17,14), a)
# print(a)
# print(kg.graph1['Alfonso Brescia'])
# print(kg.entity_dict)
